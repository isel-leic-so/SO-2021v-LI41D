...
// In x64 calling convention CurrentThread is in RDI and NextThread in RSI.
//
__attribute__ ((visibility ("hidden")))
...
void ut_exit () {
	...
	//
	// remove teminating thread from alive queue
	//
...
HANDLE ut_create (UT_FUNCTION function, UT_ARGUMENT argument) {
	...
	// 
	// insert new thread on alive list
	//
	...
	// registers that must be saved by the called (R15,R14,R13,R12,RBX, RBP)
...
BOOL ut_alive(HANDLE thread) {
	for(PLIST_ENTRY curr = alive_queue.flink; curr != &alive_queue; curr = curr->flink) {
		if (container_of(curr, UTHREAD, alive_link) == thread) return TRUE;
	}
	return FALSE;
}

void ut_join(HANDLE _thread) {
	
	PUTHREAD thread = _thread;
