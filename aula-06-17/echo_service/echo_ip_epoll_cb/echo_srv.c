...
async_handle_t server_init() {
	...
	async_handle_t listen_ah = ah_regist_fd(listenfd);
...
void on_new_connection(async_handle_t ah, void *ctx) {
	...
	int cfd = accept(ah_getfd(ah), (struct sockaddr *) &cliaddr, &clilen);
	if (cfd > 0) {
		printf("connected with %x, port %d...\n", cliaddr.sin_addr.s_addr, cliaddr.sin_port);
		async_handle_t client = ah_regist_fd(cfd);
		
		// start client read
		ah_read_async(client, on_read_request, NULL);
	}
}
...
int main(int argc, char *argv[]) {
	...
	if (ah_init() < 0) {
		fprintf(stderr, "error loop initialization\n");
		return 1;
	}
	
	// start the accept call
	ah_accept_async(server_init(), on_new_connection, NULL);
